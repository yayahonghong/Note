...
## 虚拟化 VS 容器化
| 对比项         | 虚拟化（Virtualization）         | 容器化（Containerization）      |
| -------------- | ------------------------------- | ------------------------------- |
| 启动速度       | 慢（分钟级）                    | 快（秒级）                      |
| 资源占用       | 高（需完整操作系统）             | 低（共享主机内核）              |
| 隔离性         | 强（独立操作系统）               | 较强（进程级隔离）              |
| 便携性         | 一般（依赖虚拟机管理器）         | 很好（跨平台一致性）            |
| 典型技术       | VMware、KVM、Hyper-V            | Docker、Kubernetes              |
| 适用场景       | 多操作系统、多租户环境           | 微服务、持续集成、快速部署      |

- 虚拟化通过Hypervisor在物理服务器上运行多个虚拟机，每个虚拟机有独立的操作系统。
- 容器化则在同一操作系统内核上运行多个容器，容器之间相互隔离但更轻量，启动更快，资源利用率更高。
