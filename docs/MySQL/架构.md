...
使用上一次查询的最后一条记录的主键值作为下一次查询的起点，避免大偏移量的性能问题。

```sql
SELECT * FROM tb_sku WHERE id > last_query_id ORDER BY id LIMIT 10;
```
...
```sql
select  t.*  from  tb_sku as t  ,  (select  id  from  tb_sku  order  by  id 
limit  2000000,10) as a where t.id  =  a.id;
```

创建覆盖索引

创建包含查询字段的联合索引，避免回表操作。
